...
- **P10 terminology:** Node = processing unit (e.g. `write_spec`), NodeRun = one execution record. `exec_policy.nodes` (not `jobs`) configures per-node worker, argv, and model. Registry type: `dev_process.<node_name>`.
...
- **P10 semantics:**
  - `model` in `exec_policy.nodes` is applied by `worker_adapter`. For Codex argv containing `codex exec`, `model` is injected as `--model` in the option zone only (before `--`; profile keys map to default slugs). For non-`codex exec` argv overrides (e.g. `review_argv_override`), argv is left unchanged and the resolved model is recorded for audit only. If exec policy omits `model` but argv already contains `--model`, argv is left as-is and `NodeRun.model` / evidence `model` may be `null` — `argv` in evidence is the source of truth.
  - `provider_session_id` is **strict at argv application**: it requires a Codex argv containing `codex exec`; otherwise execution fails instead of silently recording an unapplied resume request. When applied, argv uses `codex exec --model <slug> <exec flags...> resume <id>` (exec-level flags before the `resume` subcommand; passthrough after `--` is never modified). The provider may report a different applied session id; both `provider_session_id_requested` and `provider_session_id_applied` are recorded.
  - `session_id` is a **logical** id derived from `(run_id, node_name, index)`.
  - **Scope (PR5):** per-node model injection and explicit Codex provider session resume via `provider_session_id`. Automatic session reuse policies (`new_per_node_run` / `reuse_by_node`) are not implemented yet.
  - Evidence JSON includes `node_name`, `session_id`, `model`, `worker` for bidirectional traceability (`node_runs[] ↔ evidence`). Session fields: `provider_session_id_requested` + `provider_session_mode` (`resume`) when configured; `provider_session_id` / `provider_session_id_applied` from the worker response (`provider_meta.session_id`) when present — never overwrites logical `session_id`.
