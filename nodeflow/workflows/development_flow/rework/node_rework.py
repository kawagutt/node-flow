...
class ReworkPipeNode(PipeNode):
    ...
    def run(
        ...
        prepared = prepare_action_context(action="rework", inputs=inputs, params=params)
        p = prepared["params"]
        ...
        if prev_workspace_context is None:
        ...
        read_json_required(flow_cp_in, label="flow_checkpoint_path")
        rp = resume_prev.get("review_checkpoint_path")
        if not isinstance(rp, str) or not rp.strip():
            raise NodeExecutionFailure("rework requires review_checkpoint_path in flow checkpoint")
        rc = as_path(workspace, rp.strip())
        if rc is None or not rc.exists():
            raise NodeExecutionFailure(f"previous review checkpoint not found: {rp}")
        review_ctx = review_context_from_checkpoint(rc, resume_prev)
