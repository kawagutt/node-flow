...
class ApprovePipeNode(PipeNode):
    ...
    def run(
        ...
        if status == "limit":
            raise NodeExecutionLimit("development_flow approve limit")
        if status != "done":
            raise NodeExecutionFailure(f"development_flow approve unexpected status: {status}")
        return out
