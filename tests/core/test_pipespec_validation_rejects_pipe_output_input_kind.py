...
def test_pipe_output_sources_reject_kind_input() -> None:
    n = _PassNode()
    spec = PipeSpec(
        graph_node_order=("n",),
        pipe=PipeDeclaration(
            input_ports=frozenset({"request"}),
            output_sources={"bad": SourceRef(kind="input", port_name="request")},
        ),
        nodes={
            "n": NodeSpec(
                node_id="n",
    ...
    with pytest.raises(PipeSpecValidationError, match="kind='node' only"):
